...
prior to `# Sensu S.M.A.R.T. status check` from the generated README file, and add additional context about the
...
The Sensu S.M.A.R.T. status check is a [Sensu Check][6] that ...
...
metadata:
  ...
  namespace: default
spec:
  command: check-smart-status --example example_arg
  subscriptions:
  - system
  runtime_assets:
